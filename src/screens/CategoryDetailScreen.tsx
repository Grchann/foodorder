...
import { Restaurant, FoodModel, onUpdateCart, ApplicationState, UserState, ShoppingState, Category } from '../redux';
...
const _CategoryDetailScreen: React.FC<CategoryDetailProps> = (props) => {

    const { getParam, goBack } = props.navigation;

    const category = getParam('category') as Category;
    const foods = props.shoppingReducer.availableFoods.filter(food=>
        food.category.toLowerCase()===category.title.toLowerCase());
...
return (<View style={styles.container}>
        ...
        <View style={styles.body}>
            <ImageBackground source={{ uri: `${category.icon}`}}
            ...
            <View style={{ height: 120, backgroundColor: 'rgba(0,0,0,0.6)', padding: 10}}>
                ...
                <Text style={{ color: '#FFF', fontSize: 28, fontWeight: '700' }} > {category.title}</Text>
...
const CategoryDetailScreen = connect(mapToStateProps, { onUpdateCart })(_CategoryDetailScreen) 
